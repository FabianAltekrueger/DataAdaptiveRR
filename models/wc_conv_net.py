...
# SIAM Journal on Imaging Sciences 17 (1), 91-115
# (https://github.com/axgoujon/weakly_convex_ridge_regularizer)
...
class WCvxConvNet(nn.Module):
    def __init__(self, param_multi_conv, param_spline_activation, 
                param_spline_scaling, rho_wcvx=1):
        super().__init__()
        ...
        def constant_mask(inp):
            return torch.ones(inp.shape, device = self.device)
        self.mask_net = constant_mask
        self.mask = None
    ...
    def get_scaling(self, sigma=None):
        if self.scaling is None:
            eps = 1e-5
            return(torch.exp(self.spline_scaling(torch.tile(
                    sigma, (1, self.num_channels, 1, 1)))) / (sigma + eps))
        else:
            return(self.scaling)
    ...
    def set_mask_net(self, net):
        self.mask_net = net
    
    def get_mask(self, y):
        self.mask = self.mask_net(y)
    ...
    def grad_activation(self, x, sigma=None):
        scaling = self.get_scaling(sigma)
        x = x * scaling
        return(self.get_mu() * self.activation_cvx.derivative(x) - 
    ...
    def grad_denoising(self, x, x_noisy, sigma=None, cache_wx=False, lmbd=1):
        return(1/(1 + lmbd*self.get_mu()) * ((x - x_noisy) + (lmbd*
                        self.grad(x, sigma=sigma, cache_wx=cache_wx))))
