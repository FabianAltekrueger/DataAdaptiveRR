...
class ValidateCoarseToFine():
    ...
    def __init__(self, score, dir_name, exp_name, p1_init=1, p2_init=10, 
                gamma1=3, gamma2=3, decay_factor=0.5, grid_size=3, 
                freeze_p2=False, verbose=True, gamma_stop=1.05, 
                p1_max=float('inf'), p2_max=float('inf'), **kwargs):
        """ Parameters
        ----------
        score: function
            function that takes as input the parameters and return a 
            tuple (psnr, ssim, niter) corresponding to the average metrics 
            over the validation set of a method. Nb: if freeze_p2 is True, 
            the function should take only one parameter
        dir_name: string
        ...
        gamma_1_stop: float
            if defined, the stopping criterion for the first parameter, 
            otherwise gamma_stop is used
        gamma_2_stop: float
            if defined, the stopping criterion for the first parameter, 
            otherwise gamma_stop is used
        p1_max: float
        ...
        # array to store temporarily the scores on the local (grid_size x grid_size) grid
    ...
    def update_scores(self):
        ...
        for k in range(self.grid_size):
            for j in range(loop2):
                ...
                # do not compute again if such a combination is already computed
                if df_t.shape[0] > 0:
                ...
                # skip if p1 is too large
                elif (p1 > self.p1_max):
                    sk_p1 = True
                    printLine(len(self.scores))
                # skip if p2 is too large
                elif (p2 > self.p2_max):
                    sk_p2 = True
                    printLine(len(self.scores))
                # compute values for new parameters
                else:
                    ...
                    df2 = pd.DataFrame([[self.exp_name, p1, p2, psnr_t, 
                                        ssim_t, niter_t, get_time_str()]], 
